# -*- coding: utf-8 -*-
"""react-diffusion-inverse-d-500-f-20.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Y63X5ULdY5JJBGT2XKHfsM9T5vlsqbPn
...
def generate_one_ics_training_data(D_train_i, u_sol_train_i, num_points=300):
    ...
    return u_data, y, s, D_i


# Generate bcs training data corresponding to one input sample
def generate_one_bcs_training_data(D_train_i, u_sol_train_i, num_points=300):
...
# ICs data
u_ics_train, y_ics_train, s_ics_train, D_ics_train = (jax.vmap(generate_one_ics_training_data,
                                                  in_axes=(0, 0, None))
                                         (D_train, target_train, p_data_train))
...
print("nu_ics_train: ", str(D_ics_train.shape))  # Expected: (N_train, )

# BCs data
# key_bcs = jax.random.split(keys[0], n_train)
u_bcs_train, y_bcs_train, s_bcs_train, D_bcs_train = (jax.vmap(generate_one_bcs_training_data,
                                                               in_axes=(0, 0, None))
                                                    (D_train, target_train, p_data_train))
...
print("nu_bcs_train: ", str(D_bcs_train.shape))  # Expected: (N_train,)
...
ics_dataset = DataGenerator(u_ics_train, y_ics_train, s_ics_train, D_ics_train, branch_train,  batch_size, keys[0])
bcs_dataset = DataGenerator(u_bcs_train, y_bcs_train, s_bcs_train, D_bcs_train, branch_train, batch_size, keys[1])
...
save = True
params_loaded = load_model_params('/home/droysar1/data_sgoswam4/DeepONet_inverse/Reaction_Diffusion/model_params_best.pkl')
...
#     inputs, sol_ic, nu_ic = ics_batch
    inputs, sol_ic, nu_ic, fx = ics_batch
...
#     print(y[rand_num,:, 3])
    ...
    loss_s_bc0 = mse_single(s_bc1_pred)
    loss_s_bc1 = mse_single(s_bc2_pred)

    return loss_s_bc0 + loss_s_bc1
...
#     loss_value =  loss_bcs_i + 1.0 * loss_ics_i + 1.0 * loss_res_i + 1*loss_data_i
    loss_value =  10*loss_data_i + loss_bcs_i + 1 * loss_ics_i + 20 * loss_res_i
