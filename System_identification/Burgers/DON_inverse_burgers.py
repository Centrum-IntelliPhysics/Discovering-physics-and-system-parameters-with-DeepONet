...
def loss_test_l2_error_one_sample(model_fn, params_loaded, data_output, u_sol_test, idx, location_test, return_data=False):
    ...
    l2_error_data = jnp.mean(l2_norm_error(data_output_i, s_pred_known_loc))

    if return_data:
        return loss_test, l2_error, s_pred, u_sol_test[idx,:,:], l2_error_data, s_pred_known_loc, data_output_i
    else:
        return loss_test, l2_error
...
def visualize_show(model_fn_nu, model_fn, params, result_dir, epoch, data_output, u_sol_test, nu_test, idx, location_test, test=False, save=False):
    ...
    pcm1 = plt.pcolor(x_test, t_test, s_test, cmap='jet', shading='auto')
    ...
    pcm2 = plt.pcolor(x_test, t_test, s_pred, cmap='jet', shading='auto')
    ...
    plt.savefig(os.path.join(result_dir, f'Test_Sample_points_{idx}.png'),
...
def plot_error_distribution(rel_err_nu):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(result_dir, f'abs_error_nu.pdf'),
                bbox_inches='tight', dpi=300)
    plt.show()
...
def plot_error_distribution(rel_err_nu):
    ...
    # Adjust layout and display
    plt.tight_layout()
    plt.savefig(os.path.join(result_dir, f'rel_error_nu.pdf'),
                bbox_inches='tight', dpi=300)
    plt.show()
